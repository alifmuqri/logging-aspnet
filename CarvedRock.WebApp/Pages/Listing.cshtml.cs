    ...
    public class ListingModel : PageModel
    {
        ...
        public async Task OnGetAsync(){
            ...
            if(!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogWarning("API failure: {fullPath} Response: {response}, Trace: {trace}", fullPath, (int) response.StatusCode, traceId);
            ...
            if(Products.Any())
            {
                CategoryName = Products.First().Category.First().ToString().ToUpper() + Products.First().Category[1..];
            }
        }
    }
}